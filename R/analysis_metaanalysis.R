...
spect_tb_prop[23, ]
  # row 23 is the outlier


# Prepare df --------------------------------------------------------------

# mean, sd and N
lcmean_d1 <- spect_tb_prop[-23, ] %>% 
...
trt_frtl_dd_mean <- trt_frtl_dd %>%  
  filter(fileid != 211) %>% # the outlier removed above
  group_by(Site, Trt) %>%
...
# . N_added, CN_cntrl, rr_CN
mr1  <- rma.mv(yi, vi, data = rom, mod = ~ N_added + CN_cntrl + rr_CN,         random = ~1|Site)
mr2  <- rma.mv(yi, vi, data = rom, mod = ~ N_added + CN_cntrl,                 random = ~1|Site)
mr3  <- rma.mv(yi, vi, data = rom, mod = ~ N_added +            rr_CN,         random = ~1|Site)
mr4  <- rma.mv(yi, vi, data = rom, mod = ~           CN_cntrl + rr_CN,         random = ~1|Site)
AICc(mr0, mr1, mr2, mr3, mr4)
mr5  <- rma.mv(yi, vi, data = rom, mod = ~ N_added,                            random = ~1|Site)
mr6  <- rma.mv(yi, vi, data = rom, mod = ~           CN_cntrl,                 random = ~1|Site)
AICc(mr0, mr2, mr5, mr6)
summary(mr5)

# . N_year, N_rate, CN_cntrl, rr_CN
mr7   <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate + CN_cntrl + rr_CN,  random = ~1|Site)
mr8   <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate + CN_cntrl,          random = ~1|Site)
mr9   <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate +            rr_CN,  random = ~1|Site)
mr10  <- rma.mv(yi, vi, data = rom, mod = ~ N_year +          CN_cntrl + rr_CN,  random = ~1|Site)
mr11  <- rma.mv(yi, vi, data = rom, mod = ~          N_rate + CN_cntrl + rr_CN,  random = ~1|Site)
AICc(mr0, mr7, mr8, mr9, mr10, mr11)
mr12  <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate,                     random = ~1|Site)
mr13  <- rma.mv(yi, vi, data = rom, mod = ~ N_year +          CN_cntrl,          random = ~1|Site)
mr13  <- rma.mv(yi, vi, data = rom, mod = ~ N_year +                     rr_CN,  random = ~1|Site)
mr14  <- rma.mv(yi, vi, data = rom, mod = ~          N_rate + CN_cntrl,          random = ~1|Site)
mr15  <- rma.mv(yi, vi, data = rom, mod = ~          N_rate +            rr_CN,  random = ~1|Site)
mr16  <- rma.mv(yi, vi, data = rom, mod = ~                   CN_cntrl + rr_CN,  random = ~1|Site)
...
mr19  <- rma.mv(yi, vi, data = rom, mod = ~ CN_cntrl, random = ~1|Site)
mr20  <- rma.mv(yi, vi, data = rom, mod = ~ rr_CN   , random = ~1|Site)
AICc(mr0, mr17, mr18, mr19, mr20)
summary(mr0)

# . N_added, CNratio, rr_CN
mr21  <- rma.mv(yi, vi, data = rom, mod = ~ N_added + CNratio + rr_CN,         random = ~1|Site)
mr22  <- rma.mv(yi, vi, data = rom, mod = ~ N_added + CNratio,                 random = ~1|Site)
mr23  <- rma.mv(yi, vi, data = rom, mod = ~ N_added +           rr_CN,         random = ~1|Site)
mr24  <- rma.mv(yi, vi, data = rom, mod = ~           CNratio + rr_CN,         random = ~1|Site)
AICc(mr0, mr21, mr22, mr23, mr24)
mr25  <- rma.mv(yi, vi, data = rom, mod = ~ CNratio,                 random = ~1|Site)
AICc(mr22, mr25, mr5)
summary(mr5)

# . N_year, N_rate, CNratio, rr_CN
mr26 <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate + CNratio + rr_CN, random = ~1|Site)
mr27 <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate + CNratio,          random = ~1|Site)
mr28 <- rma.mv(yi, vi, data = rom, mod = ~ N_year + N_rate +           rr_CN,  random = ~1|Site)
mr29 <- rma.mv(yi, vi, data = rom, mod = ~ N_year +          CNratio + rr_CN,  random = ~1|Site)
mr30 <- rma.mv(yi, vi, data = rom, mod = ~          N_rate + CNratio + rr_CN,  random = ~1|Site)
...
mr32  <- rma.mv(yi, vi, data = rom, mod = ~ N_year +                    rr_CN,  random = ~1|Site)
mr33  <- rma.mv(yi, vi, data = rom, mod = ~          N_rate + CNratio,          random = ~1|Site)
mr34  <- rma.mv(yi, vi, data = rom, mod = ~          N_rate +           rr_CN,  random = ~1|Site)
mr35  <- rma.mv(yi, vi, data = rom, mod = ~                   CNratio + rr_CN,  random = ~1|Site)
...
plot(yi ~ f_cnr, rom)
rom %>% 
  select(Site, Trt, f_cnr, yi, rr_CN) %>% 
  arrange(f_cnr)
